...
// Function to perform DFS and find the path length
bool dfs(int current, int target, unordered_map<int, vector<int>>& tree, unordered_set<int>& visited, int& pathLength) {
    if (current == target) {
        return true;
    }
    ...
    for (int neighbor : tree[current]) {
        if (visited.find(neighbor) == visited.end()) {
            pathLength++;
            if (dfs(neighbor, target, tree, visited, pathLength)) {
                return true;
            }
            pathLength--; // backtrack
        }
    }
...
int findPathLength(int a, int b, unordered_map<int, vector<int>>& tree) {
    unordered_set<int> visited;
    int pathLength = 0;

    if (dfs(a, b, tree, visited, pathLength)) {
        return pathLength;
    }

    return -1; // If no path found, though in a tree there should always be a path
}
...
void solve() {
    ...
    int pathLength = findPathLength(a, b, tree);
    int max_h=dfsHeight
